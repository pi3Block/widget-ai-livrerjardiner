"""Exceptions spécifiques au module Product."""

from typing import Optional, List

class ProductDomainException(Exception):
    """Classe de base pour les exceptions du module Product."""
...
class CategoryNotFoundException(ProductDomainException):
    ...
    def __init__(self, category_id: int, message: Optional[str] = None):
        detail = message or f"Catégorie avec ID {category_id} non trouvée."
        super().__init__(detail)
        self.category_id = category_id

class TagNotFoundException(ProductDomainException):
    """Levée lorsqu'un tag spécifique n'est pas trouvé."""
    def __init__(self, tag_id: Optional[int] = None, message: Optional[str] = None):
        detail = message or f"Tag avec ID {tag_id} non trouvé."
        super().__init__(detail)
        self.tag_id = tag_id
...
class InvalidOperationException(ProductDomainException):
    """Levée pour une opération invalide (ex: SKU dupliqué, suppression impossible, nom dupliqué)."""
    pass

class ProductUpdateFailedException(ProductDomainException):
    """Levée lorsque la mise à jour d'un produit échoue pour une raison autre que 'non trouvé'."""
    pass
